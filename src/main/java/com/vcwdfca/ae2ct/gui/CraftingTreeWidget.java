...
public class CraftingTreeWidget {
    ...
    public void draw(GuiGraphics guiGraphics, @SuppressWarnings("unused") int offsetX,
        ...
        for (LegacyTreeLayout.Entry entry : layout.entriesInRange(range.minColumn(), range.maxColumn(),
            range.minRow(), range.maxRow(), renderAnchors())) {
    ...
    private void ensureReady() {
        ...
        try {
            ...
            LOGGER.error("Failed to build fallback legacy crafting tree", e);
            dataFuture = null;
            return;
        }
    ...
    private void drawAmount(GuiGraphics guiGraphics, GenericStack stack, int x, int y, int color) {
        ...
        var text = getDrawAmount(stack.what(), stack.amount());
        var fontX = font.width(text);
        var fontY = font.lineHeight;
        float scale = 0.4f;
        poseStack.scale(scale, scale, scale);
        guiGraphics.drawString(font, text, (x + 18 - fontX * scale) / scale,
